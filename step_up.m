function out = step_up(curr,j,M)
%description: paket move towards up one step
global pack;
global node;
global inuse;
global net;
...
out = 0;
if node(temp).down_full == 0 && inuse(curr,temp) == 0
    pack(j).src_y = pack(j).src_y - 1;
    node(temp).down = node(temp).down + 1;
    node(temp).down_pack = [node(temp).down_pack j];
    pack(j).move = 1;
