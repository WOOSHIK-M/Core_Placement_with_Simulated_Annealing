function out = step_right(curr,j)
...
temp = curr + 1;
out = 0;
if node(temp).left_full == 0 && inuse(curr,temp) == 0
    pack(j).src_x = pack(j).src_x + 1;
    node(temp).left = node(temp).left + 1;
    node(temp).left_pack = [node(temp).left_pack j];
    pack(j).move = 1;
