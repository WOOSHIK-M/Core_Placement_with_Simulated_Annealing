function out = chip_step_up_1(curr_node,j,CHIP_M,N)
...
if chip(temp_chip).down_full == 0 && inuse_chip(curr_chip,temp_chip) == 0
    pack(j).move = 1;
    pack(j).src_y = N-1;
    ...
    chip(temp_chip).down = chip(temp_chip).down + 1;
    chip(temp_chip).down_pack = [chip(temp_chip).down_pack j];
