function out = chip_step_down_2(curr_chip,j,NODE_NUM,CHIP_M,M,N)
...
last_node = (curr_chip-CHIP_M-1)*NODE_NUM + pack(j).src_x + (N-1) * M + 1;
out = 0;

if node(temp_node).up_full == 0 && inuse(last_node,temp_node) == 0
    busy = 1;
    pack(j).move = 1;
    if pack(j).cross_time < 10
        pack(j).cross_time = pack(j).cross_time + 1;
    else
        node(temp_node).up = node(temp_node).up + 1;
        node(temp_node).up_pack = [node(temp_node).up_pack j];
        pack(j).cross_time = 0;
