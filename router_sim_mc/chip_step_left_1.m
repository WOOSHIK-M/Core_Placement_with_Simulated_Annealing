function out = chip_step_left_1(curr_node,j,M)
...
if chip(temp_chip).right_full == 0 && inuse_chip(curr_chip,temp_chip) == 0
    pack(j).move = 1;
    pack(j).src_x = M-1;
    ...
    chip(temp_chip).right = chip(temp_chip).right + 1;
    chip(temp_chip).right_pack = [chip(temp_chip).right_pack j];
