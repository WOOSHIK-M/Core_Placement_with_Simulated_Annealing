function out = chip_step_up_2(curr_chip,j,NODE_NUM,CHIP_M,M)
...
last_node = (curr_chip+CHIP_M-1)*NODE_NUM + pack(j).src_x + 0 + 1;
out = 0;

if node(temp_node).down_full == 0 && inuse(last_node,temp_node) == 0
    busy = 1;
    pack(j).move = 1;
    if pack(j).cross_time < 10
        pack(j).cross_time = pack(j).cross_time + 1;
    else
        node(temp_node).down = node(temp_node).down + 1;
        node(temp_node).down_pack = [node(temp_node).down_pack j];
        pack(j).cross_time = 0;
